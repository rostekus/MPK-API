...
class LineNameModel():
    ...
    def find_routeTable_by_id(self, lineId):
        ...
        for Table in tDirectionTables.findAll('td', recursive=False):
            tDirection = Table.find(
                'div', {'class': 'headSign'}).contents[0]  # route direction
            ...
            for Row in Rows[1:]:  # skipping table header
                ...
                dStreetStop.update({sStreetStop: {
                                   'direction': sDirection, 'stopNumber': sNumber, 'timeTableId': sTimeTableId}})
...
def getLineNameIds():
    ...
    for lineType in dLineTypes:
        ...
        for dataRow in tData.find_all('a'):
            ...
            lineId = dataRow.get('href').partition(
                '?')[2].partition('&')[0].partition('=')[2]
...
class TimeTableModel():
    ...
    def _get(self, url, direction, lineName, timetableId, stopNumber):
        """
        ...
        dDayTypeNames = {
            dtname.get_text(): dtname.get('id').replace(
                '_name', '') for dtname in dTab.find(
                'div', {
                    'id': 'dDayTypeNames'}).find_all('a')}
        dDayTypes = dTab.find("div", {"id": "dDayTypes"})

        dDayTables = {}
        for dName, tName in dDayTypeNames.items():
            dDayTables[dName] = dDayTypes.find(
                "div", {"id": "table_{}".format(tName)}).find('table')
    ...
    def get_bus_table(cls, lineName, directionName, stopName):
        ...
        table = TimeTableModel()._get(url, direction, lineName, timetableId, stopNumber)
        return table
